...
export default cds.service.impl(function () {
  ...
  async function getCurrentLeave(tx, req) {
    return await tx.run(SELECT.one.from(Leave).where({ ID: req.data.ID }));
  }
  ...
  async function leaveValidations(req, leave, employee) {
    ...
    if (!leave.approver_ID)
      return req.reject(400, "There is no approver to the leave");

    if (leave.approver_ID !== employee.ID && !req.user.is("HR"))
      return req.reject(403, "You cannot act on this leave");

    if (employee.status !== "active")
      return req.reject(400, "Approver Inactive");
  ...
  async function getEmployeeDashboard(tx, req, employee) {
    const pendingLeaves = await tx.run(
      SELECT.one.from(Leave).columns`count(*) as count`.where({
    ...
    const balanceLeaves = await tx.run(
      SELECT.one
        .from(LeaveBalance)
        .columns("sickBalanceRemaining", "casualBalanceRemaining")
    ...
    const upcomingHolidays = await tx.run(
      SELECT.one.from(Holiday).columns`count(*) as count`.where({
        holidayDate: { ">=": today },
      }),
    );
  ...
  async function getManagerDashboard(tx, req, employee) {
    const teamSize = await tx.run(
      SELECT.one.from(Employee).columns`count(*) as count`.where({
  ...
  this.before("CREATE", Leave, async (req) => {
    ...
    const requestedDays = getInclusiveDayCount(
      normalizedStartDate,
      normalizedEndDate,
    );
    ...
    if (
      req.data.leaveType === "sick" &&
      balance.sickBalanceRemaining < requestedDays
    ) {
      return req.reject(400, "Insufficient leave balance");
    }
  ...
  this.before("READ", Leave, async (req) => {
    ...
    if (req.user.is("Manager")) {
      const managerEmployees = await tx.run(
        SELECT.from(Employee).columns("ID").where({ manager_ID: employee.ID }),
      );
  ...
  this.on("approveLeave", async (req) => {
    ...
    const balance = await tx.run(
      SELECT.one.from(LeaveBalance).where({ employee: leave.employee_ID }),
    );
    if (!balance) return req.reject(404, "Leave balance not found.");

    const leaveDays = getInclusiveDayCount(leave.startDate, leave.endDate);

    // Updating Casual Leaves in LeaveBalance
    if (leave.leaveType === "casual") {
      if (balance.casualBalanceRemaining < leaveDays)
        return req.reject(400, "Insufficient leave balance.");

      await tx.run(
        UPDATE(LeaveBalance)
          .set({
            casualBalanceRemaining: balance.casualBalanceRemaining - leaveDays,
          })
    ...
    if (leave.leaveType === "sick") {
      if (balance.sickBalanceRemaining < leaveDays)
        return req.reject(400, "Insufficient leave balance");

      await tx.run(
        UPDATE(LeaveBalance)
          .set({
            sickBalanceRemaining: balance.sickBalanceRemaining - leaveDays,
          })
          .where({ employee: leave.employee_ID }),
      );
    }
  ...
  this.on("cancelLeave", async (req) => {
    ...
    if (leave.employee_ID !== employee.ID)
      return req.reject(403, "You can only cancel your own leave");
  ...
  this.before("CREATE", Holiday, async (req) => {
    ...
    if (req.user.is("HR")) {
      ...
      if (!holidayName || !holidayDate)
        return req.reject(400, "Holiday either has no name or no date");
  ...
  this.before("UPDATE", Holiday, async (req) => {
    ...
    if (req.user.is("HR")) {
      ...
      const holiday = await tx.run(SELECT.one.from(Holiday).where({ ID: ID }));
  ...
  this.on("getDashboard", async (req) => {
    ...
    if (req.user.is("Employee"))
      return await getEmployeeDashboard(tx, req, employee);

    if (req.user.is("Manager"))
      return await getManagerDashboard(tx, req, employee);
